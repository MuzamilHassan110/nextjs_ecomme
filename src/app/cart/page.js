        ...
        <div className='m-5 px-20'>
            {cartCount > 0 ? (
                <>
                    ...
                    <button onClick={deleteProduct} className='text-orange-400 mt-2 font-bold hover:text-red-600 hover:cursor-pointer'>
                    ...
                    cartItem.map((item, index) => (
                        ...
                        <div className='flex justify-between border rounded-md p-4 my-2 bg-white hover:shadow-lg'>
                            <Link href={`products/${item.id}`} className='flex items-center'>
                                <Image src={item.image} alt='image' className='w-20 h-auto' width={200} height={200} />
                                <p className='font-semibold text-xl ml-2'>{item.name}</p>
                            </Link>
                            <div className='flex items-center gap-5'>
                                <div className='flex items-center gap-3'>
                                    <button onClick={() => dececrementProduct(item.id)} className='p-1 rounded-md text-orange-500 hover:bg-orange-500 hover:text-white disabled:cursor-not-allowed'>
                                        <MinusIcon className='w-6 h-6' />
                                    </button>
                                    <p className='font-semibold text-xl'>{ item.quantity }</p>
                                    <button onClick={() => IncrementProduct(item.id)} className='p-1 rounded-md text-orange-500 hover:bg-orange-500 hover:text-white disabled:cursor-not-allowed'>
                                        <PlusIcon className='w-6 h-6' />
                                    </button>
                                </div>
                                <p> <span className='text-xl font-semibold'>{(item.price/100)* item.quantity}</span></p>
                                <button onClick={() => {deleteProductById(item.id)}} className='text-orange-500 hover:text-red-600'>
